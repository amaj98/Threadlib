...
void send(int tid, char *msg, int len){
	...
	{
		tmptcb = q->tcb;
        if(tmptcb->thread_id == tid){
            if(tmptcb->messages == NULL){
                tmptcb->messages = tmp;
            }
            else{   
                messageNode* tmpmsg = tmptcb->messages;
...
void receive(int* tid, char *msg, int *len){
    // int waiting = 1;
        messageNode* tmp = running->messages;
        tmp = running->messages;
        if(*tid == 0){
            if(tmp!=NULL){
                int i;
                *len = tmp->len;
                ...
                *len = tmp->len;
                ...
                while(tmp->next != NULL){
                    if(tmp->next->sender == *tid){
                        *len = tmp->next->len;
                        int i;
                        for(i = 0; i<tmp->next->len; i++){
                            msg[i] = tmp->message[i];
                        }
                        tcb* free_tmp = tmp->next;
